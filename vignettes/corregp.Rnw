...
set). Because the data are arranged in a frequency table (by crossing
...
in case of conditional variables in \texttt{part}), the resampling is more
specifically done with \emph{multinomial sampling}. Any frequency table can
be thought of as a multinomial variable, and the division of the cell counts
by the total sample size \texttt{N} gives natural estimates of the
individual cell probabilities. The estimated cell probabilities and the
sample total \texttt{N} are passed as arguments to the built-in R function
...
Each of these replicated tables lead to bootstrap/simulated replicates of
